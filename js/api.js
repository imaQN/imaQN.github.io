...
function getMatche(dateFrom, dateTo){
    ...
    if ('caches' in window){
        caches.match(`${base_url}competitions/2000/matches`)
    ...
    fetchAPI(`${base_url}competitions/2000/matches`)
        .then(data => {
            setTimeout( () =>{
                displayMatch(data, dateFrom, dateTo);
            }, 400)
...
function getTeams(group){
    ...
    if ('caches' in window){
        caches.match(`${base_url}competitions/2000/standings`)
        .then(response => {
            if(response){
                response.json()
                .then(data => {
    ...
    fetchAPI(`${base_url}competitions/2000/standings`)
        .then(data => {
            setTimeout( () =>{
                displayTeams(data, group);
            }, 400)
...
function displayProfile(data){
    ...
    data.squad.forEach(player =>{
        ...
        if (positionRole === null)
            positionRole = player.role.toLowerCase();
